...
use std::io;
use html::LoreHtml;
use parser::file::{file_from_text_to_lore, file_from_lore_to_html};

/// 处理单个文件：从 .lore 文件生成 .html 文件
fn process_lore_file(src: &Path, dst: &Path) -> io::Result<()> {
    println!("处理文件: {:?}", src);
...
pub fn map_lore_files(src_dir: &Path, dst_dir: &Path) -> io::Result<()> {
    ensure_dir(dst_dir)?;
    ...
    for entry in fs::read_dir(src_dir)? {
        ...
        if src_path
            ...
            let dst_path = dst_dir
                .join(rel_path);
            map_lore_files(&src_path, &dst_path)?;
        } else {
            // 只处理 .lore 文件
            if src_path
                .extension()
                .and_then(|ext| ext.to_str()) == Some("lore")
            {
                ...
                process_lore_file(&src_path, &dst_path)?;
            }
        }
    }
...
fn main() -> io::Result<()> {
    ...
    map_lore_files(src_dir, dst_dir)?;
